...
class ModalityFusion(nn.Module):
    ...
    def forward(self, seq_feat, img_feat, ref_pad_mask=None):
        ...
        if self.mode == "train":
            ...
            kl = 0.5 * torch.mean(
                torch.exp(log_sigma) + torch.square(mu) - 1.0 - log_sigma
            )
            output["latent"] = z
            output["kl_loss"] = kl
            seq_feat_[:, 0] = z
