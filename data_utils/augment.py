...
def render_svg(svg_str, font_dir, char_idx, aug_idx, img_size):
    ...
    svg_path.close()
    cairosvg.svg2png(
        url=f"{font_dir}/aug_svgs/{str(char_idx)}.svg",
        write_to=f"{font_dir}/aug_imgs/{str(char_idx)}_{aug_idx}.png",
        ...
        for i in range(
            ...
            for j in range(opts.n_chars):
                ...
                for k in range(opts.n_aug):
                    ...
                    img_arr = render_svg(
                        char_seq_aug, font_dir, j, aug_idx=k, img_size=opts.img_size
                    )
                    ret_img_list[k].append(img_arr)

            for k in range(opts.n_aug):
                ret_seq_list[k] = np.array(ret_seq_list[k]).reshape(
                    opts.n_chars, opts.max_len * 10
                )
                ret_img_list[k] = np.array(ret_img_list[k]).reshape(
                    opts.n_chars, opts.img_size, opts.img_size
                )
                np.save(
                    os.path.join(font_dir + "_" + str(k), f"sequence.npy"),
                    ret_seq_list[k],
                )
                np.save(
                    os.path.join(
                        font_dir + "_" + str(k), f"rendered_{opts.img_size}.npy"
                    ),
                    ret_img_list[k],
                )
...
def main():
    ...
    parser.add_argument(
        "--n_aug", type=int, default=5, help="for each font, augment it for n_aug times"
    )
