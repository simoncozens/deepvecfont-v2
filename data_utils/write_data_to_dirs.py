...
def create_db(opts, output_path, log_path):
    ...
    def process(process_id):
        ...
        cur_process_log_file = open(
            os.path.join(log_path, f"log_{opts.split}_{process_id}.txt"), "w"
        )
        for i in range(
            ...
            if not os.path.exists(
                os.path.join(cur_font_sfd_dir, "imgs_" + str(opts.img_size) + ".npy")
            ):
                continue
            ...
            for char_id in range(num_chars):
                ...
                if not os.path.exists(
                    os.path.join(
                        cur_font_sfd_dir,
                        "{}_{num:0{width}}.sfd".format(
                            font_id, num=char_id, width=num_chars_w
                        ),
                    )
                ):
                    break
                ...
                if not svg_utils.is_valid_path(pathunibfp):
                    msg = (
                        f"font {font_idx}, char {char_idx}'s sfd is not a valid path\n"
                    )
                    cur_process_log_file.write(msg)
                    char_desp_f.close()
                    sfd_f.close()
            ...
            if len(cur_font_glyphs) == num_chars:
                ...
                rendered = np.load(
                    os.path.join(
                        cur_font_sfd_dir, "imgs_" + str(opts.img_size) + ".npy"
                    )
                )
                ...
                if not os.path.exists(
                    os.path.join(
                        output_path, "{num:0{width}}".format(num=i, width=num_fonts_w)
                    )
                ):
                    os.mkdir(
                        os.path.join(
                ...
                np.save(
                    os.path.join(
                        output_path,
                        "{num:0{width}}".format(num=i, width=num_fonts_w),
                        "rendered_" + str(opts.img_size) + ".npy",
                    ),
                    rendered,
                )
    ...
    def process(process_id, return_dict):
        ...
        for i in range(
            ...
            for charid in range(num_chars):
                ...
                cur_sum_count = mean_stddev_accum.add_input(
                    cur_sum_count, cur_font_char
                )
        return_dict[process_id] = cur_sum_count

    processes = [
        mp.Process(target=process, args=[pid, return_dict])
