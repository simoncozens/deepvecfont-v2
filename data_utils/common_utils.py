...
def affine_shear(seq, dx=-0.3, dy=0.0):
    ...
    seq_args = seq_12[:, 4:]
    seq_args = np.concatenate(
        [seq_args[:, :2], seq_args[:, 2:4], seq_args[:, 4:6]], 0
    ).transpose(1, 0)
    affine_matrix = np.array([[1, dx], [dy, 1]])
    rotated_args = np.dot(affine_matrix, seq_args)
    rotated_args = rotated_args.transpose(1, 0)
    new_args = np.concatenate(
        [rotated_args[:71], rotated_args[71:142], rotated_args[142:]], -1
    )
    new_args[:, 0] += 12.0
