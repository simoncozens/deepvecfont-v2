...
def write_glyph_imgs_mp(opts):
    ...
    def process(process_id, font_num_p_process):
        for i in range(
            process_id * font_num_p_process, (process_id + 1) * font_num_p_process
        ):
            if i >= font_num:
                break
            ...
            fontimgs_array = np.zeros(
                (len(charset), opts.img_size, opts.img_size), np.uint8
            )
            fontimgs_array[:, :, :] = 255
            ...
            for charid in range(len(charset)):
                # read the meta file
                txt_fpath = os.path.join(
                    sfd_path,
                    opts.split,
                    fontname,
                    fontname
                    + "_"
                    + "{num:0{width}}".format(num=charid, width=charset_lenw)
                    + ".txt",
                )
                try:
                ...
                try:
                    ...
                    print(
                        "cant calculate height and width "
                        + "%04d" % i
                        + "_"
                        + "{num:0{width}}".format(num=charid, width=charset_lenw)
                    )
                ...
                draw_pos_y = (
                    ...
                    + opts.img_size
                    - ascent
                    - int((opts.img_size / 24.0) * (4.0 / 3.0))
                )
                # else:
                ...
                if opts.debug:
                    image.save(
                        os.path.join(
                            ...
                            opts.split,
                            fontname,
                            str(charid) + "_" + str(opts.img_size) + ".png",
                        )
                    )
            ...
            if flag_success:
                np.save(
                    os.path.join(
                        ...
                        opts.split,
                        fontname,
                        "imgs_" + str(opts.img_size) + ".npy",
                    ),
                    fontimgs_array,
                )

    processes = [
        mp.Process(target=process, args=(pid, font_num_per_process))
