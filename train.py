...
def train_main_model(opts):
    ...
    parameters_all = [
        ...
        {"params": model_main.img_decoder.parameters()},
        {"params": model_main.modality_fusion.parameters()},
        {"params": model_main.transformer_main.parameters()},
    ...
    for epoch in range(opts.init_epoch, opts.n_epochs):
        for idx, data in enumerate(train_loader):
            ...
            loss = (
                opts.loss_w_l1 * loss_dict["img"]["l1"]
            ...
            if batches_done % opts.freq_log == 0:
                ...
                if opts.tboard:
                    ...
                    writer.add_scalar(
                        "Loss/img_kl_loss",
                        opts.kl_beta * loss_dict["kl"].item(),
                        batches_done,
                    )
                    writer.add_image(
                        "Images/trg_img", ret_dict["img"]["trg"][0], batches_done
                    )
                    writer.add_image(
            ...
            if opts.freq_sample > 0 and batches_done % opts.freq_sample == 0:
                ...
                save_file = os.path.join(
                    dir_sample, f"train_epoch_{epoch}_batch_{batches_done}.png"
                )
        ...
        if epoch % opts.freq_ckpt == 0:
            if opts.multi_gpu:
                torch.save(
                    {
                        "model": model_main.module.state_dict(),
                        "opt": optimizer.state_dict(),
                        "n_epoch": epoch,
                        "n_iter": batches_done,
                    },
                    f"{dir_ckpt}/{epoch}_{batches_done}.ckpt",
                )
            else:
                torch.save(
                    {
                        "model": model_main.state_dict(),
                        "opt": optimizer.state_dict(),
                        "n_epoch": epoch,
                        "n_iter": batches_done,
                    },
